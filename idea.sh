...
BITS=""
if [ -z "$JRE" ] && [ -s "${CONFIG_HOME}/JetBrains/IdeaIC2021.2/idea.jdk" ]; then
...
if [ -n "$IDEA_VM_OPTIONS" ] && [ -r "$IDEA_VM_OPTIONS" ]; then
...
elif [ -r "${CONFIG_HOME}/JetBrains/IdeaIC2021.2/idea${BITS}.vmoptions" ]; then
  # 3. <config_directory>/<bin_name>.vmoptions
  VM_OPTIONS_FILE="${CONFIG_HOME}/JetBrains/IdeaIC2021.2/idea${BITS}.vmoptions"
else
  ...
  if [ -r "${CONFIG_HOME}/JetBrains/IdeaIC2021.2/user.vmoptions" ]; then
    if [ -n "$VM_OPTIONS_FILE" ]; then
      VM_OPTIONS="${CONFIG_HOME}/JetBrains/IdeaIC2021.2/user.vmoptions"
    else
      USER_VM_OPTIONS_FILE="${CONFIG_HOME}/JetBrains/IdeaIC2021.2/user.vmoptions"
    fi
  fi
fi
